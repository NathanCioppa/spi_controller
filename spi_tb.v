...
initial begin
	...
	$display("Testing that spi_periferal recieves correct word for all patterns ...");
	for(i=0; i<2**WORD_SIZE;i=i+1)
		spi_periferal_recieve_test(i);
...
task spi_periferal_recieve_test;
...
begin
	...
	@(posedge clk); #1;
	start = 0;
	wait(!controller_busy); #100000;

	if(out_exp !== in_perif0_word)
		$display("FAILED spi_periferal_recieve_test. expected: %d. Got: %d", out_exp, in_perif0_word);
end
endtask
